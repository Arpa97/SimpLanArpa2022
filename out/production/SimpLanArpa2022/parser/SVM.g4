...
LOADRA	 : 'lra' ;	// load  ra in the top of stack     $ra<-$sp        ste:sono come li ho capiti io
STORERA  : 'sra' ;	// store top of stack into ra       $sp<-$ra

LOADRV	 : 'lrv' ;	// load rv in the top of stack
STORERV  : 'srv' ;	// store top of stack into rv

LOADFP	 : 'lfp' ;	// load fp in the top of stack      $fp<-$sp
STOREFP	 : 'sfp' ;	// store top of stack in fp         $sp<-$fp

LOADHP	 : 'lhp' ;	// load hp in the top of stack
STOREHP	 : 'shp' ;	// store top into $hp

STORER1  : 'sr1' ; // store top of stack into r1        $r1<-$sp
LOADR1   : 'lr1' ; // load r1 into the top of stack     $sp<-$r1

STORER2  : 'sr2' ; // store top into r2                 $r2<-$sp
LOADR2   : 'lr2' ; // load r2 into the top of stack     $sp<-$r2

STOREAL : 'sal' ; // store top into al                  $al<-$sp
LOADAL :  'lal' ; // load al into the top of stack      $sp<-$al


// -------------- LI ISTRUCTIONS -------------
LIR1        : 'lir1' ; //set r1 with the value number n         $r1<-n
LIR2        : 'lir2' ; //set r2 with the value number n         $r2<-n
